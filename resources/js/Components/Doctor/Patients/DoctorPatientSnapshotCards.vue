...
import type {DoctorPatientOverviewSnapshot} from '@/lib/doctor/patients/buildDoctorPatientOverviewSnapshot';
import {
    dailyMoodPresentation
    
} from '@/lib/mood/dailyMoodPresentation';
import type {DailyMoodPresentation} from '@/lib/mood/dailyMoodPresentation';
import {
    medicationIntakeDayPresentation
    
    
} from '@/lib/patient/medications/history/medicationIntakeDayPresentation';
import type {MedicationIntakeDayIconStatusValue, MedicationIntakeDayPresentation} from '@/lib/patient/medications/history/medicationIntakeDayPresentation';
    ...
    <div class="flex min-w-0 flex-col gap-3">
        ...
        <div class="grid min-w-0 gap-4 md:grid-cols-2 md:gap-5">
            ...
            >
                ...
                <ul v-else class="grid min-w-0 grid-cols-3 gap-2 md:gap-3">
                    <li
                    ...
                    >
                        <button
                            ...
                            :aria-label="
                                t(
                                    'doctor.patients.snapshotMedicationStatusSelect',
                                    {
                            ...
                            "
                            @click="
                                emit('selectMedicationStatus', option.status)
                            "
                        >
                            ...
                            <span
                                :class="
                                    cn(
                                        'text-xl leading-none font-semibold tabular-nums md:text-2xl',
                                        option.faceClass,
                                    )
                                "
                            >
