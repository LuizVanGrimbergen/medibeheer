...
test('linked family members see wellbeing check-ins on updates', function () {
    ...
    $response = $this->actingAs($familyUser)->get(route('family.updates'));
...
test('family members without a patient link see empty wellbeing on updates', function () {
    $familyUser = User::factory()->familyMember()->create();
...
test('linked family members can visit family wellbeing', function () {
    ...
    $response->assertOk();
    $response->assertInertia(fn ($page) => $page->component('Family/Updates'));
});
...
test('family updates calendar only loads check-ins for the requested month', function () {
