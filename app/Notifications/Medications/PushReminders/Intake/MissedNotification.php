...
final class MissedNotification extends Notification
{
    ...
    public function toWebPush(object $notifiable, Notification $notification): WebPushMessage
    {
        ...
        return (new WebPushMessage)
            ->title(PatientMedicationReminderTranslations::trans('patient_medication_reminders.missed_notification.title'))
            ->body(PatientMedicationReminderTranslations::trans('patient_medication_reminders.missed_notification.body', [
                'name' => $name,
                'type' => $typeLabel,
            ]))
            ->icon(url('/images/medibeheer-pwa.png'))
            ->tag(sprintf(
                'medication-intake-missed-%d-%s',
                (int) ($this->slot['medication_schedule_id'] ?? 0),
                (string) ($this->slot['dose_time'] ?? ''),
            ))
            ->data([
                'openUrl' => route('patient.dashboard'),
            ]);
    }
}
